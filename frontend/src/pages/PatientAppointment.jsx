...
export default function PatientAppointment() {
  ...
  const fetchAppointments = async () => {
    try {
      const user = JSON.parse(localStorage.getItem("user"));
      const response = await api.get(`/patient/${user._id}/appointments`);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-10 py-8 md:py-12">
      <h2 className="text-xl md:text-2xl font-black text-gray-900 mb-6 md:mb-8">My Appointments</h2>
      {appointments.length === 0 && (
        <div className="bg-white p-8 md:p-12 rounded-3xl text-center shadow-sm">
          <h3 className="text-lg md:text-xl font-bold text-gray-700">No appointments found</h3>
          <p className="text-sm md:text-base text-gray-500 mt-2">Book your first appointment to see it here.</p>
        </div>
      )}
      
      {appointments.map((appt) => (
        <div key={appt._id} className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm mb-6 flex flex-col md:flex-row justify-between items-start md:items-center relative">
          <div 
            className="flex flex-col md:flex-row gap-6 md:gap-8 cursor-pointer flex-1 w-full" 
            onClick={() => navigate(`/appointment/${appt._id}/live`, { state: { appointment: appt } })}
          >
            <div>
            ...
            <div className="md:border-l border-gray-100 md:pl-8 flex flex-col gap-2 justify-center pt-4 md:pt-0 border-t md:border-t-0">
          ...
          <button onClick={() => setMenuOpen(menuOpen === appt._id ? null : appt._id)} className="absolute top-6 right-6 md:static p-2 hover:bg-gray-100 rounded-full">
            <FiMoreVertical />
          </button>
          {menuOpen === appt._id && (
            <div className="absolute right-6 top-16 md:right-10 md:top-16 bg-white shadow-xl rounded-2xl border border-gray-100 p-2 z-10 w-48">
              ...
              <button onClick={() => alert("Cancellation request submitted.")} className="flex items-center w-full px-4 py-3 text-xs font-bold text-red-500 hover:bg-red-50 rounded-xl"><FiXCircle className="mr-2" /> Cancel</button>
            </div>
          )}
        </div>
      ))}
      ...
      {showInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-2xl rounded-3xl p-6 md:p-10 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <button onClick={() => setShowInvoice(null)} className="absolute top-6 right-6 text-gray-400 hover:text-gray-600"><FiXCircle size={20} /></button>

            <h2 className="text-lg md:text-xl font-black text-center mb-2">Medical Clinic Invoice</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8 border-t border-b py-6">
              <div className="space-y-2">
                <p className="text-xs font-black text-gray-800">Invoice Details:</p>
                <p className="text-[10px] font-bold text-gray-500">• Invoice Number: {showInvoice.invoice_number || showInvoice._id}</p>
                <p className="text-[10px] font-bold text-gray-500">• Date of Issue: {new Date(showInvoice.created_at).toLocaleDateString()}</p>
              </div>
              <div className="space-y-2">
                <p className="text-xs font-black text-gray-800">Bill To:</p>
                <p className="text-[10px] font-bold text-gray-500">• Patient Name: {showInvoice.patient_name}</p>
                <p className="text-[10px] font-bold text-gray-500">• Contact Number: {showInvoice.phone}</p>
              </div>
            </div>
