...
export default function Login(props) {
  ...
  return (
    <div className="min-h-screen bg-cover bg-center relative flex flex-col items-center justify-center p-4"
      ...
      {showPopup && (
        <div className="fixed top-8 z-50 bg-[#0b645b] text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-3 animate-bounce">
          <FiCheckCircle className="text-lg" />
          <span className="text-sm font-bold">Successfully {isRegistering ? "registered" : "logged in"}!</span>
        </div>
      )}

      {/* Main container - Responsive width management */}
      <div className="relative z-10 w-full max-w-sm flex flex-col gap-6">
        <div className="rounded-3xl bg-white p-6 sm:p-8 shadow-2xl relative w-full">
          <button onClick={() => navigate("/")} className="absolute right-4 top-4 sm:right-6 sm:top-6 text-gray-300 hover:text-gray-600">
            <FiX size={20} />
          </button>
          ...
          <h1 className="text-center text-xl sm:text-2xl font-black text-slate-900">{isRegistering ? "Create Account" : "MediCare Patient"}</h1>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4 sm:space-y-5">
            {isRegistering && (
              <div>
                ...
                <div className="flex items-center rounded-2xl border bg-gray-50 px-4 h-11 sm:h-12">
                  <FiUser className="text-gray-400" />
                  <input required name="fullName" type="text" placeholder="Enter your full name" className="ml-3 w-full bg-transparent outline-none text-xs sm:text-sm" />
                </div>
              </div>
            )}
            <div>
              ...
              <div className="flex items-center rounded-2xl border bg-gray-50 px-4 h-11 sm:h-12">
                ...
                <input required name="email" type="email" placeholder="Enter your email" className="ml-3 w-full bg-transparent outline-none text-xs sm:text-sm" />
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center rounded-2xl border bg-gray-50 px-4 h-11 sm:h-12">
                ...
                <input
                  ...
                  className="ml-3 w-full bg-transparent outline-none text-xs sm:text-sm"/>
              </div>
            </div>
            <button type="submit" className="flex h-12 w-full items-center justify-center rounded-2xl bg-[#0b645b] text-sm font-bold text-white hover:bg-[#084e46] transition active:scale-[0.98]">
              {isRegistering ? "Register" : "Login"} <FiArrowRight className="ml-2" />
            </button>
          </form>
          
          <div className="mt-6 text-center text-[11px] sm:text-xs font-bold text-gray-500">
