...
export default function PatientReport() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-10 py-12 animate-in fade-in">
      <h2 className="text-2xl font-black text-gray-900 mb-8">Medical Reports</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {reports.map((report) => (
          <div key={report.id} className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm flex justify-between items-center hover:shadow-md transition-all">
            <div className="flex gap-6 items-center">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">{report.type} • {report.id}</p>
                <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-[10px] font-bold text-gray-500">
      ...
      {previewReport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-2xl rounded-3xl p-10 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <button onClick={() => setPreviewReport(null)} className="absolute top-8 right-8 text-gray-400 hover:text-gray-600"><FiXCircle size={20}/></button>
            
            <div className="border-b pb-6 mb-4">
              <h1 className="text-xl font-black text-[#0b645b] uppercase">MediCare Pathology Lab</h1>
              <p className="text-[10px] font-bold text-gray-400">123 Health Ave, New Delhi • Contact: 011-23456789</p>
            </div>

            <div className="grid grid-cols-2 gap-4 text-[10px] mb-6 bg-gray-50 p-4 rounded-xl">
            ...
            <h3 className="font-black text-sm mb-2 uppercase">{previewReport.title}</h3>
            ...
            <p className="text-[10px] italic text-center font-bold text-gray-400 mb-4">****End of Report****</p>
            
            <div className="flex justify-between pt-8 border-t text-[10px] font-bold text-gray-400">
              <p className="text-center">Lab Technician<br/>(DMLT)</p>
              <p className="text-center">Pathologist<br/>(MD, Path)</p>
            </div>
            
            <button className="mt-8 w-full py-3 bg-[#0b645b] text-white rounded-xl font-black text-xs">DOWNLOAD PDF</button>
          </div>
        </div>
      )}
    </div>
